	...
	ready: function() {
		...
		this.container.on( 'click', 'button.repeater-add', function( e ) {
			...
			if ( ! limit || control.currentIndex < limit ) {
				...
				jQuery('.limit').css('background', 'none');
				jQuery('.limit').css('color', '#000');
				var url = 'https://www.famethemes.com/plugins/onepress-plus/?utm_source=theme_customizer&utm_medium=text_link&utm_campaign=onepress_customizer#get-started';
				jQuery('.repeater-add').css('display','none');
				var b = 'lois';
				jQuery('.limit').html('Upgrade to <a href="' + url + '">' + b + '</a> to be able to add more items and unlock other premium features!');
			}
		});
